...
// token layer (src/index.css) only. Scans every .tsx file under src/ and
...
if (violations.length > 0) {
  console.error(`${violations.length} raw hex color literal(s) found in .tsx files (DESIGN.md: hex belongs in the token layer only):\n`);
  for (const v of violations) console.error(`  ${v.file}:${v.line}  ${v.match}`);
  process.exit(1);
}

console.log('No raw hex color literals found in .tsx files.');
